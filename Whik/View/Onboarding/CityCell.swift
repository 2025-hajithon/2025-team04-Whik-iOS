...
final class CityCell: UICollectionViewCell {
    ...
    func configure(with city: City, selected: Bool) {
        thumbnailImageView.image = UIImage(named: "sampleImage")
        nameLabelKR.text = city.name
        nameLabelEN.text = city.englishName

        contentView.layer.borderWidth = selected ? 2 : 0
        contentView.layer.borderColor = selected ? UIColor.whikPrimary.cgColor : nil
        
        contentView.setNeedsLayout()
        contentView.layoutIfNeeded()
        
        /// 그라데이션 레이어 다시 한번 업데이트
